...
Expr expr_parse_with_precedence(Line_Context* ctx, BinOprPrec p)
{
    ...
    while (bin_opr_get_def(tok.type, &dets) && dets.prec == p) {
        log_to_ctx(ctx, LOG_FORMAT("[BINOP]", "[EXPR]", "found bin op %d", dets.type));
...
Expr expr_peek_next(Line_Context* ctx)
{
    ...
    case TOKEN_TYPE_NAME:
        {
            if (compare_str(token.text, STR("true"))) {
                ...
                if (next.type == TOKEN_TYPE_OPEN_PAREN) {
                    log_to_ctx(ctx, LOG_FORMAT("[IDENTIFICATION]", "[EXPR]", "funcall: '%.*s'", Str_Fmt(token.text)));
    ...
    default:
        {
            log_to_ctx(ctx, LOG_FORMAT("[IDENTIFICATION]", "[STMT]", "got %s %.*s", token_get_name(token.type), Str_Fmt(token.text)));
