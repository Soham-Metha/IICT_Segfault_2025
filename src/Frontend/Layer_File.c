...
Error file_read(const char *file_path, File_Context *file)
{
	...
	ERROR_CHECK(out, goto cleanup, AST_generate(&global, true));
	ERROR_CHECK(out, goto cleanup,  IR_generate(&global));
...
Line_Context *file_fetch_next_line(File_Context *file)
{
	...
	log_to_ctx(curr, LOG_FORMAT, LOG_CTX("","--xx--"), file->contents.len);
	log_to_ctx(curr, LOG_FORMAT "\"%.*s\"", LOG_CTX("[READING]", "[L%3u]"),
			Str_Fmt(curr->line), file->line_num);
