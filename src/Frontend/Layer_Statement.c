...
StmtConditional* get_stmt_conditional(Line_Context* ctx)
{
    ...
    assert(res->cond.type == EXPR_TYPE_BIN_OPR || res->cond.type == EXPR_TYPE_BOOL || res->cond.type == EXPR_TYPE_NUMBER);
...
TypeList* parse_typelist(Line_Context* ctx)
{
    ...
    update_indent(1);
    log_to_ctx(ctx, LOG_FORMAT("[IDENTIFICATION]", "[STMT]", "- Parameters:", ""));

    update_indent(1);
    token = token_peek_next(ctx);
    if (token.type == TOKEN_TYPE_CLOSING_PAREN) {
        token_consume(ctx);
        log_to_ctx(ctx, LOG_FORMAT("[IDENTIFICATION]", "[STMT]", " NO ARGS !", ""));
        update_indent(-2);
        return NULL;
    }
    ...
    do {
        ...
        if (!token_consume(ctx)) {
            print(ctx, WIN_STDERR, " ERROR: expected %s or %s\n", token_get_name(TOKEN_TYPE_CLOSING_PAREN), token_get_name(TOKEN_TYPE_COMMA));
            exit(1);
        }
...
static inline Stmt __TOKEN_TYPE_NAME(Token tok, Line_Context* ctx)
{
    ...
    log_to_ctx(ctx, LOG_FORMAT("[IDENTIFICATION]", "[STMT]", "variable: '%.*s'", Str_Fmt(tok.text)));
...
Stmt stmt_fetch_next(Line_Context* ctx)
{
    ...
    default:
        ...
        log_to_ctx(ctx, LOG_FORMAT("[IDENTIFICATION]", "[STMT]", "%s %.*s", token_get_name(tok.type), Str_Fmt(tok.text)));
