...
{
	print(WIN_AST, AST("ellipse", "lightgoldenrod1", "%.*s"), id,
	      Str_Fmt(value));
	return id;
}
...
static int AST_dump_statement(const Stmt *stmt, int *n, int *b)
{
	...
	switch (stmt->type) {
	case STMT_VARIABLE:
		return __STMT_VARIABLE(myId, stmt->value.as_var);
	case STMT_LIT_INT:
		return __STMT_LIT_INT(myId, stmt->value.as_int);
	case STMT_LIT_FLOAT:
		return __STMT_LIT_FLOAT(myId, stmt->value.as_float);
	case STMT_LIT_CHAR:
		return __STMT_LIT_CHAR(myId, stmt->value.as_char);
	case STMT_LIT_STR:
