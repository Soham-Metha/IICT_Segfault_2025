...
varType dump_var_decl(String var_nm, String type, int id)
{
    ...
    case VAR_TYPE_STR:
        {
            print_IR(IR_FORMAT("; declaring var:   %.*s     ", Str_Fmt(var_nm)));
...
varType dump_var_defn(Block_Context_IR* ctx, String var_nm, varType type, TypeList* list)
{
    ...
    case VAR_TYPE_BOOL:
        ...
        break;
    case VAR_TYPE_PTR:
        assert(0 && "ptr cant be initialized using the '=' syntax");
    case VAR_TYPE_VOID:
        return VAR_TYPE_VOID;
    case VAR_TYPE_CNT:
    default:
...
void dump_typelist(Block_Context_IR* ctx, TypeList* list)
{
    ...
    for (int i = list->count - 1; i >= 0; i--) {
        ...
        if (type == VAR_TYPE_PTR) {
            dump_typelist(ctx, v->args);
            print_IR(IR_FORMAT("PUSH    E_%d", get_var_details(ctx, v->name).mem_addr));
            print_IR(IR_FORMAT("PUSH    E_%d", get_var_details(ctx, v->args->var[0].name).mem_addr));
            print_IR(IR_FORMAT("WRITE8", ""));
        }
        if (v->has_init) {
            StmtNode nxt             = (StmtNode) { .next = NULL, .statement = *v->init };
...
varType IR__STMT_FUNCALL(Block_Context_IR* ctx, const Funcall* funcall)
{
    ...
    if (dets.list) {
        int i = 0;
        for (const FuncallArg* arg = funcall->args; arg != NULL; arg = arg->next) {
            varType argtype      = IR_dump_expr(ctx, arg->expr);
            varType expectedtype = get_type_details_from_type_name(dets.list->var[i].type).type;
...
static varType IR_dump_expr(Block_Context_IR* ctx, Expr expr)
{
    ...
    case EXPR_TYPE_BIN_OPR:
        ...
        print_IR(IR_FORMAT("%s", bin_opr_inst_LUT[l][expr.as.bin_opr->type].inst));
